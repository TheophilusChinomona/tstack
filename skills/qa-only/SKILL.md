...
preamble-tier: 4
version: 1.0.0
description: Report-only QA testing. (tstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
1. **Project-scoped test plans:** Check `./docs/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   setopt +o nomatch 2>/dev/null || true  # zsh compat
   eval "$( 2>/dev/null)"
   ls -t ./docs/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
...
3. Copy report template from `skills/qa/templatesqa-report-template.md` to output dir
...
Then follow the **per-page exploration checklist** (see `skills/qa/references/issue-taxonomy.md`):
...
**Write each issue to the report immediately** using the template format from `skills/qa/templatesqa-report-template.md`.
...
```
Write to `./docs/projects/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
...
```bash
 '{"skill":"qa-only","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
