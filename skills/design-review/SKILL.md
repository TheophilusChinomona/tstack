...
preamble-tier: 4
version: 2.0.0
description: "Designer's eye QA: finds visual inconsistency, spacing issues, hierarchy problems, AI slop patterns, and slow interactions — then fixes them. (tstack)"
allowed-tools:
  - Bash
  - Read
  - Write
...
```bash
jq -n --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg skill "SKILL_NAME" --arg branch "$(git branch --show-current 2>/dev/null)" --arg insight "ONE_LINE_SUMMARY" '{ts:$ts,skill:$skill,branch:$branch,insight:$insight}' >> ./docs/analytics/eureka.jsonl 2>/dev/null || true
```
...
3. Create directory structure (test/, skills/spec/, etc.)
...
MUST be saved to `./docs/projects/$SLUG/designs/`, NEVER to `.context/`,
...
```bash
eval "$( 2>/dev/null)"
REPORT_DIR="./docs/plans/designs/design-audit-$(date +%Y%m%d)"
...
if [ "$_CROSS_PROJ" = "true" ]; then
   --limit 10 --cross-project 2>/dev/null || true
else
...
```
Write to: `./docs/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
```
./docs/projects/$SLUG/designs/design-audit-{YYYYMMDD}/
...
eval "$( 2>/dev/null)" && mkdir -p ./docs/projects/$SLUG
```
Write a one-line summary to `./docs/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
...
```bash
 '{"skill":"design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
