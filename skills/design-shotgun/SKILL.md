...
preamble-tier: 2
version: 1.0.0
description: "Design shotgun: generate multiple AI design variants, open a comparison board, collect structured feedback, and iterate. (tstack)"
triggers:
...
allowed-tools:
  ...
  - AskUserQuestion
    ...
    - id: recent-design-docs
      kind: filesystem
      glob: "./docs/projects/{repo_slug}/*-design-*.md"
      sort: mtime_desc
      limit: 3
...
_PREV=$(find ./docs/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -5)
...
```bash
setopt +o nomatch 2>/dev/null || true
ls ./docs/projects/$SLUG/*office-hours* 2>/dev/null | head -5
```
...
**Persistent taste profile (v1 schema at `./docs/projects/$SLUG/taste-profile.json`):**

Read the persistent taste profile if it exists:

```bash
_TASTE_PROFILE=./docs/projects/$SLUG/taste-profile.json
...
the legacy approved.json aggregate — ``
...
```bash
setopt +o nomatch 2>/dev/null || true
_TASTE=$(find ./docs/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -10)
```
...
variant, call ` approved <variant-path>`. When they
explicitly reject a variant, call ` rejected <variant-path>`.
...
**Why /tmp/ then cp?** In observed sessions, `$D generate --output ./docs/...`
...
1. **Never save to `.context/`, `docs/designs/`, or `/tmp/`.** All design artifacts go
   to `./docs/projects/$SLUG/designs/`. This is enforced. See DESIGN_SETUP above.
