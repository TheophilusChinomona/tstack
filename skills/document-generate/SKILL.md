...
preamble-tier: 2
version: 1.0.0
description: Generate missing documentation from scratch for a feature, module, or entire project. (tstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
```markdown
# [Tutorial title — describes what you'll buildlearn]
...
```bash
REDACT_VIS=$( get redact_repo_visibility 2>/dev/null)
[ -z "$REDACT_VIS" ] && REDACT_VIS=$(gh repo view --json visibility -q .visibility 2>/dev/null | tr 'A-Z' 'a-z')
git diff --cached --no-color | grep '^+' | sed 's/^+//' | \
   --repo-visibility "${REDACT_VIS:-unknown}" --json
