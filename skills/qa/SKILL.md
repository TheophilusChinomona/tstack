...
preamble-tier: 4
version: 2.0.0
description: Systematically QA test a web application and fix bugs found. (tstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
| running the QA baseline (Phases 1-6) — mode selection (Diff-aware/Full/Quick/Regression), the phase-by-phase browser workflow, the Health Score Rubric, framework-specific guidance, and the browser-testing Important Rules | `sectionsqa-patterns.md` |
...
if [ "$_CROSS_PROJ" = "true" ]; then
   --limit 10 --query "qa testing bug regression flake fixture" --cross-project 2>/dev/null || true
else
   --limit 10 --query "qa testing bug regression flake fixture" 2>/dev/null || true
fi
```
...
```bash
 '{"skill":"qa","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
