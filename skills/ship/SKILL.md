...
preamble-tier: 4
version: 1.0.0
description: "Ship workflow: detect + merge base branch, run tests, review diff, bump VERSION, update CHANGELOG, commit, push, create PR. (tstack)"
allowed-tools:
  - Bash
  - Read
  - Write
...
| the pre-landing review and specialist dispatch (Step 9) | `sectionsreview-army.md` |
...
**STOP and Read `./sections/apple-release.md` FIRST**
...
For Design Review: run `source <( <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running design-review for a full visual audit post-implementation." Still never block.
...
3. **Queue-aware pick** (workspace-aware ship):
   ```bash
   QUEUE_JSON=$(bun run  --base <base> --bump "$BUMP_LEVEL" --current-version "$BASE_VERSION" 2>/dev/null || echo '{"offline":true}')
...
5. **Record the release decision** (durable cross-session memory). The bump level is a real decision the next session should not re-derive blind:
   ```bash
    '{"decision":"Ship NEW_VERSION (BUMP_LEVEL)","rationale":"WHY","scope":"repo","source":"skill","confidence":9}' 2>/dev/null || true
   ```
...
Read `.claude/skillsskills/review/TODOS-format.md` for the canonical format reference.
...
```bash
 check --label tests --expect-cmd '<exact tests-lane command from Step 5>' --label vitest --expect-cmd '<exact vitest-lane command from Step 5>' --max-age 24 --allow-paths CHANGELOG.md,VERSION,package.json
```
...
- **Any STALE/MISSING (exit non-zero):** run live, wrapped, so the fresh run is
  recorded: ` run --label <lane> -- '<command>'`.
...
2. **`REDACT_PREPUSH` not true AND `PREPUSH_PROMPTED: no`** — one-time
   ...
   If A: run ` set redact_prepush_hook true`
   then ` install-prepush-hook`.
   If B: run ` set redact_prepush_hook false`.
...
Route the append through `review-log`. It resolves the project slug and
...
```bash
 '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"'"$(git rev-parse --abbrev-ref HEAD)"'"}'
```
...
```bash
_NUDGE_MARKER="./docs/.plan-tune-nudge-shown"
...
`rm ./docs/.plan-tune-nudge-shown` before next ship.
