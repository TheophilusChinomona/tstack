...
preamble-tier: 3
version: 1.0.0
description: CEO/founder-mode plan review. (tstack)
allowed-tools:
  - Read
  - Grep
  - Glob
...
No special preamble required. Proceed with the skill workflow.
...
1. **`CONDUCTOR_SESSION: true` echoed** → do NOT call AskUserQuestion at all: render EVERY decision brief as the **prose form** below and STOP.
...
_LOCALDOC=$(ls -t ./docs/plans/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$_LOCALDOC" ] && _LOCALDOC=$(ls -t ./docs/plans/*-design-*.md 2>/dev/null | head -1)
# Repo-local docs win when at least as fresh (#703): office-hours dual-writes
# docs/designs/ directory, and the committed copy is what teammates
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
HANDOFF=$(ls -t ./docs/plans/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
Read the `/office-hours` skill file using the Read tool.
...
```bash
mkdir -p ./docs/plans/ceo-plans/archive
# For each stale plan: mv ./docs/plans/ceo-plans/{old-plan}.md ./docs/plans/ceo-plans/archive/
```

Write to `./docs/plans/ceo-plans/{date}-{feature-slug}.md` using this format:
...
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ./docs/analytics/spec-review.jsonl 2>/dev/null || true
```
...
```
NOTE: These represent human-team implementation hours. With AI + tooling,
...
> **STOP.** Before running the 11-section deep review, required outputs, and review report (only after Step 0 scope and mode are agreed), Read the `sections/review-sections.md` file and execute it
