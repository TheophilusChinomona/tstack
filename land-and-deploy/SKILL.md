...
preamble-tier: 4
version: 1.0.0
description: Land and deploy workflow. (tstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
```bash
eval "$( 2>/dev/null)"
if [ ! -f ./docs/projects/$SLUG/land-deploy-confirmed ]; then
  echo "FIRST_RUN"
else
  # Check if deploy config has changed since confirmation
  SAVED_HASH=$(cat ./docs/projects/$SLUG/land-deploy-confirmed 2>/dev/null)
